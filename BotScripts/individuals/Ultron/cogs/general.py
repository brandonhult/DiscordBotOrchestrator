...
from BotScripts.individuals.Ultron.features.helpers import (
...
from BotScripts.individuals.Ultron.features.llm import generate_ultron_reply_sync
...
class General(commands.Cog):
    def __init__(self, bot, conversation_governor):
        self.bot = bot
        self.conversation_governor = conversation_governor

        self.interject_bias = 0.28
        self.dropout_chance = 0.50
    ...
    async def milk(self, ctx):
        logger.info("%s used %s", ctx.author, ctx.message.content)
        await ctx.send("Milk? That's a weak man's drink.")
    ...
    def _should_auto_reply(self, message: discord.Message) -> tuple[bool, str | None]:
        ...
        if (
            ...
            and self._has_pending_followup(message)
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            is_direct = reason == "direct"
            is_trigger = reason == "trigger"

            if not message.author.bot:
                self.conversation_governor.register_human_message(channel_id)

            allowed = self.conversation_governor.can_bot_reply(
                channel_id=channel_id,
                bot_id=self.bot.user.id,
                base_chance=self.interject_bias,
                is_direct=is_direct,
            ...
            if reason == "trigger":
                ...
                trigger_line = dynamic_entities[0]["text"] if dynamic_entities else target_name
                reply = f"{trigger_line}. Yes. I heard that."

                sent = await message.reply(reply, mention_author=False)
                self.conversation_governor.register_bot_reply(
                    channel_id=channel_id,
                    bot_id=self.bot.user.id,
            ...
            logger.exception("Ultron auto-reply failed: %s", e)


async def setup(bot):
    conversation_governor = getattr(bot, "conversation_governor", None)
    if conversation_governor is None:
        raise RuntimeError("Bot is missing shared conversation_governor")
    await bot.add_cog(General(bot, conversation_governor))