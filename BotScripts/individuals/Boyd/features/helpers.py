...
from BotScripts.individuals.Boyd.data.static.listener import ACTIVATORS
from BotScripts.individuals.Boyd.data.static.user_config import DISCORD_ALIASES
...
def member_variants(member: discord.Member) -> list[dict[str, Any]]:
    ...
    variants = [
        ...
        {"text": f"The one they call {name}...", "member_id": member.id, "name": name},
        {"text": f"That one called {name}...", "member_id": member.id, "name": name},
        {"text": f"{name}. I've seen that name before...", "member_id": member.id, "name": name},
        {"text": f"{name}... that's not a normal name.", "member_id": member.id, "name": name},
